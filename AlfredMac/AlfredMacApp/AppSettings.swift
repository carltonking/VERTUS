...
//  AlfredMacApp
//
//  Ported from the iOS app (Alfred/Alfred/Services/AppSettings.swift). Keeps
//  the same property names — isConfigured, socketHost, socketPort, socketURL,
//  relayEndpoint, relayToken — and the same UserDefaults keys, so the two
//  surfaces speak the same language. Deviation: the iOS app stores `token` in
//  the Keychain; this companion shell keeps it in UserDefaults (the existing
//  menu-bar app already reads relayEndpoint/relayToken from UserDefaults).
//
...
private nonisolated enum SettingsKeys {
    ...
    /// The relay the existing menu-bar app reads (see startRelayIfConfigured
    /// in Alfred/App/AlfredApp.swift) — kept under the same keys so a user who
    /// configured the relay once has it answered here too.
    static let relayEndpoint = "relayEndpoint"
    static let relayToken = "relayToken"
}

/// The port the iOS client uses for the live socket, mirrored here so the
/// companion's default matches the phone's (AlfredWebSocketClient.defaultPort).
private let defaultSocketPort = 8766
...
public final class AppSettings {
    ...
    /// The API token. UserDefaults here (the iOS app keeps it in the Keychain;
    /// this shell keeps everything in UserDefaults for simplicity).
    var token: String {
        didSet { UserDefaults.standard.set(token, forKey: SettingsKeys.token) }
    }
    ...
    /// NYU coursework (mirrors the Mac's NYUIntegrationManager settings).
    var nyuEnabled: Bool {
    ...
    /// The relay endpoint/token the existing menu-bar app answers phone
    /// requests through. Same keys as the menu-bar app's AppDelegate.
    var relayEndpoint: String {
        didSet { UserDefaults.standard.set(relayEndpoint, forKey: SettingsKeys.relayEndpoint) }
    }

    var relayToken: String {
        didSet { UserDefaults.standard.set(relayToken, forKey: SettingsKeys.relayToken) }
    }
    ...
    public init() {
        ...
        relayEndpoint = UserDefaults.standard.string(forKey: SettingsKeys.relayEndpoint) ?? ""
        relayToken = UserDefaults.standard.string(forKey: SettingsKeys.relayToken) ?? ""
    }
    ...
    /// an AppSettings — which would read the real UserDefaults.
    ///
