...
//  Alfred
//
//  The phone's own calendar, read and written through EventKit.
//
//  Alfred's cloud brain reaches iCloud over CalDAV (api/_lib/caldav.ts) because a cron job at 7am
//  can't ask a sleeping phone anything. Drawing a list on screen is the opposite case: the events
//  are already on the device, across every account the user has added — iCloud, Google, Exchange,
//  subscribed feeds — so going phone → server → iCloud → back would show *less* and take longer.
//  That server path pins a single calendar via CALDAV_CALENDAR_URL; this one sees all of them.
//
//  The store is also the app's write side: creating, editing, and deleting events lands straight
//  into EventKit, and the Calendar tab watches for changes everywhere else so the screen stays
//  honest whether the change came from Alfred or from Apple Calendar on another device.
//
...
/// A group of calendars under one account (iCloud, Google, Exchange, "On My iPhone"…), for the
...
final class CalendarStore {
    /// iOS's answer reduced to the three cases the UI draws differently. `writeOnly` collapses into
    ...
    private static func currentAccess() -> Access {
        ...
        case .writeOnly:
            return .denied(reason: "Alfred can add events to your calendar but not read them. "
                + "Switch Calendars to Full Access in Settings to see them here.")
        case .denied:
            return .denied(reason: "Calendar access is turned off for Alfred.")
        case .restricted:
            return .denied(reason: "Calendar access is restricted on this device, so Alfred can't read it.")
        @unknown default:
            return .denied(reason: "iOS gave an answer about calendar access this app doesn't recognise.")
        }
    }

    /// Ask iOS for access, then load. Only ever called from an explicit tap: `RootView` builds every
    /// tab up front, so requesting on appear would throw the system prompt at launch from a tab the
    /// user hasn't opened.
    ...
    /// EventKit's own notification that something changed — an event added on the Mac, a new
    /// account synced. Without this the tab would keep showing whatever it read when it opened.
    ///
    /// iOS fires `.EKEventStoreChanged` in bursts — a calendar syncing can produce several within
