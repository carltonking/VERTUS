...
//  AlfredMacApp
//
//  The Mac's reminders, read and written through EventKit — EventKit keeps reminders in the
...
//
//  Ported from the iOS app (Alfred/Alfred/Services/RemindersStore.swift). EventKit's macOS 14 API
//  (`requestFullAccessToReminders`, `authorizationStatus(for:)`) matches the iOS 17 API, so the
//  store is unchanged except for wording that no longer assumes a phone.
//
    ...
    private static func currentAccess() -> Access {
        ...
        case .writeOnly:
            return .denied(reason: "Alfred can add reminders but not read them. "
                + "Switch Reminders to Full Access in System Settings to see them here.")
        case .denied:
            return .denied(reason: "Reminders access is turned off for Alfred.")
        case .restricted:
            return .denied(reason: "Reminders access is restricted on this Mac, so Alfred can't read them.")
        @unknown default:
            return .denied(reason: "macOS gave an answer about reminders access this app doesn't recognise.")
        }
    }
