...
//  Alfred
//
//  The iOS half of the Mac mail contract. These mirror the macOS wire shapes
//  in AlfredMac/Alfred/Mail/MailManager.swift exactly — the phone decodes
//  `mail.*` results with these, and encodes nothing back (actions are plain
//  parameter dictionaries). Keep the two in lockstep.
//
...
/// returned whole by `mail.message` so the phone makes one call per open.
