...
//
//  Two halves, matching what macOS itself allows:
//  • Accounts & Calendars — every calendar EventKit can see, grouped by the account it came from
//    (iCloud, Google, Exchange, On My Mac…), each with a visibility toggle. Accounts themselves
//    are added in System Settings, the one place Apple lets that happen.
//  • Subscriptions — feeds (webcal:// or .ics) that Alfred fetches on the backend and mirrors
//    into an EventKit calendar, since there's no public API for an app to subscribe to a feed.
//
//  Ported from the iOS app (Alfred/Alfred/Views/Calendar/AccountsCalendarsView.swift).
//  Deviations: "Add Account" opens the macOS Internet Accounts pane, and subscription removal
//  moves from swipe actions to a context menu (macOS has no swipe actions).
//
...
struct AccountsCalendarsView: View {
    ...
    private var accountsSection: some View {
        Section {
            ...
            Text("Every calendar on this Mac appears here, wherever it came from. Turn one off and it hides from every Alfred view — Home briefing included.")
        }
    }
    ...
    private var subscriptionsSection: some View {
        Section {
            ...
            Text("Alfred's server fetches each feed and the app mirrors it into an \"Alfred Subscriptions\" calendar. Sync pulls the feed fresh; events removed from the feed are removed here.")
        }
    }
    ...
    private func subscriptionRow(_ sub: CalendarSubscription) -> some View {
        ...
        // macOS has no swipe actions — removal lives on the context menu.
        .contextMenu {
    ...
    private var addAccountSection: some View {
        Section {
            ...
            Text("Accounts are added in System Settings — Alfred can't create them itself, but everything you add there appears here automatically.")
        }
        .alert("Add in Settings", isPresented: $showSettingsHint) {
            ...
            Text("macOS only lets accounts be added from System Settings → Internet Accounts. Do that once, and every calendar it brings (iCloud, Google, Exchange…) shows up in Alfred.")
        }
    }
}
