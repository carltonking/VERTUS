...
//  snippet, date, unread dot, star), the swipe gestures Mail trained everyone
//  to expect, and above it all the natural-language search bar and the subtle
//  classification chips the Mac's copilot produces.
//
//  Works in both panes of the split view: with a `selection` binding (iPad)
//  tapping a row selects it into the reader column; without one (iPhone) rows
//  push onto the navigation stack.
//
//  Swipes act immediately and reconcile afterwards (see MacMailStore) — over a
//  phone connection an IMAP round-trip is long enough that a gesture which
//  waits for it reads as a broken gesture. Archiving shows the "Archived ·
//  Undo" toast, and Undo moves the message back to the inbox.
//
...
struct MailListView: View {
    ...
    /// account dropdown below writes it back (so the two agree on iPad).
    let scope: MailSidebarItem
    /// iPad: the row → reader selection. nil on iPhone, where rows push.
