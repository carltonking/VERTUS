...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	...
	console.log(chalk.green(`${APP_NAME} is already up to date (v${BRAND_VERSION})`));
			...
			case "update": {
				...
				if (updateTargetIncludesSelf(target)) {
					...
					if (managedInstallRoot) {
						...
						console.log(chalk.green(`Updated ${APP_NAME} from ${BRAND_VERSION} to ${selfUpdatePlan.version}`));
						return true;
					}
