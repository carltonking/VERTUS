 ...
 *
 * The server sets VERTUS_REMOTE=1 when the prompt comes from a
...
export default function (pi: ExtensionAPI) {
	pi.on("before_agent_start", async (event) => {
		if (process.env.VERTUS_REMOTE !== "1") return; // local: trusted
	...
	pi.on("tool_call", async (event) => {
		if (process.env.VERTUS_REMOTE !== "1") return undefined;
		...
		if (DESTRUCTIVE_PATTERNS.some((re) => re.test(command))) {
			return {
				block: true,
				reason:
					"VERTUS approval guard: destructive command blocked in remote session. Run it locally or ask the user to approve it.",
			};
		}
		return undefined;
	});
