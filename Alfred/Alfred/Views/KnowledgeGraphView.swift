...
//      glance).
//    • Canvas — the graph itself: pan, pinch-zoom, tap a node to see its
//      summary, neighbors and layers.
//
// Everything comes from the Mac over the socket (`code.understand_*`) and is
// computed there from .ua/knowledge-graph.json — the phone never sees the raw
// graph JSON.
...
/// zoom and tap-to-inspect. Layout is a circle by degree (the busiest nodes on
/// top, the focused center dead-center), which reads as "architecture" without
/// a force simulation on the phone.
