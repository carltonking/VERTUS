...
//  Alfred
//
//  The mail copilot contract, shared with the iOS app. These mirror the macOS
//  wire shapes in AlfredMac/Alfred/Mail/MailAIService.swift exactly — this app
//  decodes `mail.classify` / `mail.summarize` / `mail.extract_tasks` /
//  `mail.draft_reply` / `mail.search_ai` results with these.
//
//  The payloads are deliberately small: the model does the heavy reading on
//  the Mac, and the app just renders what came back (plus whatever it cached
...
/// The app's mirror of the Mac's MailSettings. The settings screen edits
