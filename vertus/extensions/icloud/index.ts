/**
 * VERTUS iCloud Extension — Calendar (EventKit) + Mail (Mail.app scripting).
 *
 * Gives the VERTUS CLI read/write access to the user's calendars and email
 ...
 *
 * Design:
 *  - Calendar: a small EventKit CLI (`src/vertus-events.swift`) compiled once
 *    to ~/.vertus/agent/bin/vertus-events on first use. ALL calendar mutations are
 *    sandboxed in native code to a dedicated calendar (default "VERTUS") that
 ...
 *
 * Config: ~/.vertus/agent/icloud.json  →  { "calendarName": "VERTUS" }
 */
...
// ── Paths & config ──────────────────────────────────────────────────────────
const AGENT_DIR = path.join(os.homedir(), ".vertus", "agent");
const BIN_DIR = path.join(AGENT_DIR, "bin");
const HELPER_BIN = path.join(BIN_DIR, "vertus-events");
...
const HELPER_SRC = path.join(PKG_DIR, "src", "vertus-events.swift");
...
function loadConfig(): Config {
	try {
		...
		return { calendarName: typeof raw.calendarName === "string" && raw.calendarName.trim() ? raw.calendarName.trim() : "VERTUS" };
	} catch {
		return { calendarName: "VERTUS" };
	}
}
...
async function ensureHelper(): Promise<string> {
	if (fs.existsSync(HELPER_BIN)) {
		...
		const needRebuild = [HELPER_SRC, path.join(AGENT_DIR, "extensions", "icloud", "src", "vertus-events.swift")]
...
async function runOSA(script: string, payload: unknown, timeoutMs = 90_000): Promise<Record<string, unknown>> {
	...
	const file = path.join(tmpDir, `vertus-mail-${process.pid}-${osaCounter++}.js`);
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		description:
			"List all calendars visible to this Mac (iCloud, Exchange, Gmail, local…). Read-only — use to discover calendar titles, then query or create events. Mutations always go to the dedicated 'VERTUS' calendar, never real ones.",
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			...
			calendar: Type.Optional(Type.String({ description: "Restrict to one calendar title (e.g. 'VERTUS', 'Personal', 'School'). Default: all calendars." })),
	...
	const calendarMutationsGuideline =
		"Calendar mutations (create/cancel) are sandboxed to the dedicated 'VERTUS' calendar and enforced in native code — never create or cancel events in the user's real calendars.";
	...
	pi.registerCommand("icloud-status", {
		description: "Check VERTUS calendar + email integration (helper build, permissions, config)",
