...
//  Alfred
//
//  Calendar subscriptions: feeds the user pasted a URL for (webcal:// or .ics), which iOS itself
//  cannot subscribe to — that flow is Settings-only and Apple-owned. Alfred's backend fetches and
//  parses the feed (api/feed.ts), and this store mirrors the events into a local EventKit calendar
//  named "Alfred Subscriptions", so they show up in every Calendar view and the Home briefing just
//  like native events.
//
//  The phone owns the subscription list; the backend is a stateless fetcher. Each sync is
...
/// One subscription the user added, as stored on the phone.
...
final class SubscriptionStore {
    ...
    /// The writable EventKit calendar the mirror lives in, created on first use. iOS always has a
    /// local source ("On My iPhone") to put it in, so it never has to borrow someone else's account.
    private func ensureMirrorCalendar() throws -> EKCalendar {
        ...
        new.cgColor = UIColor.systemBlue.cgColor
