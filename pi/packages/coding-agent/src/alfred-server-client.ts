// ALFRED server client for one-shot CLI prompts.
//
// When the ALFRED hub (alfred/server/alfred_server.py) is reachable, a
// one-shot `alfred "<text>"` proxies through it so the CLI shares the same
...
//
// Env overrides:
//   ALFRED_SERVER_URL  base URL of the hub (default http://127.0.0.1:8787)
//   ALFRED_NO_SERVER=1 bypass the hub entirely
...
function serverUrl(): string | undefined {
	if (process.env.ALFRED_NO_SERVER === "1") return undefined;
	return process.env.ALFRED_SERVER_URL ?? DEFAULT_SERVER_URL;
}

async function readToken(): Promise<string | undefined> {
	try {
		const raw = await readFile(join(homedir(), ".alfred", "token"), "utf8");
 ...
 */
export async function tryRunViaAlfredServer(options: ServerOneShotOptions): Promise<boolean> {
	...
	try {
		...
		process.stderr.write("\n⚠️ lost connection to the ALFRED hub\n");
		process.exitCode = 1;
	}
	return !hadError;
}
