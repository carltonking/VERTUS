...
//  pick the project folder, type what you want built, and send. The agent runs
//  on the Mac and streams back into CodeSessionView live.
//
//  Ported from Alfred/Alfred/Views/CodeView.swift — same layout and palette.
//  Deviations: no dictation (the iOS composer's mic is phone-side; on the Mac
//  you just type), and swipe-to-delete becomes a context menu.
//
...
struct CodeView: View {
    ...
    private var projectSubtitle: String {
        let path = settings.selectedProjectPath.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !path.isEmpty else { return "Choose where on your Mac Alfred should work" }
        return path
    }
    ...
    private var promptCard: some View {
        VStack(spacing: 12) {
            TextField("What do you want me to code?", text: $prompt, axis: .vertical)
                ...
                .font(.system(size: 16))
                .foregroundStyle(palette.textPrimary)
            // Return inserts a newline, the iMessage pattern — prompts are
            // multi-line, and Send is one click away.
    ...
    /// iMessage-style send: a filled circle that lifts the prompt off the screen
