...
/// Wire events from the ALFRED hub (alfred/server/alfred_server.py).
enum AlfredEvent: Equatable {
    case text(String)
...
/// Thin client for the ALFRED hub: POST /api/prompt + GET /api/events (SSE).
final class AlfredClient {
    ...
    func send(_ text: String, onEvent: @escaping (AlfredEvent) -> Void) {
        streamTask?.cancel()
        streamTask = Task { [weak self] in
            guard let self else { return }
            let events = await self.openEventStream(onEvent: onEvent)
            guard let events else {
                onEvent(.error("Could not reach the ALFRED hub"))
                return
            }
            do {
    ...
    private func postPrompt(_ text: String) async throws -> Bool {
        ...
        req.setValue("Bearer \(token)", forHTTPHeaderField: "Authorization")
        req.setValue("phone", forHTTPHeaderField: "X-Alfred-Client")
    ...
    private func openEventStream(
        onEvent: @escaping (AlfredEvent) -> Void
    ) async -> AsyncThrowingStream<AlfredEvent, Error>? {
