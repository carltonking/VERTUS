...
//  AlfredMacApp
//
//  Ported from the iOS app (Alfred/Alfred/Services/AlfredClient.swift).
//  The cloud relay client — one POST to Alfred's JSON front door (api/mac.ts).
//  Settings' "Test connection" proves the address, token and brain end to end.
//
...
struct AlfredClient {
    ...
    /// alive request — Alfred reading the calendar, then calling a model — isn't killed by the app
