...
//  classification row, the collapsible ✨ summary, and the Summarize / Draft
//  Reply / Extract Tasks toolbar — loads asynchronously after the body,
//  because the model reads on the Mac.
//  Ported from the iOS app (Alfred/Alfred/Views/Mail/MailReaderView.swift).
//
        ...
        VStack(alignment: .leading, spacing: 10) {
            if detail.hasHTML {
                ...
                MailBodyWebView(
                    ...
                    palette: palette,
                    height: $bodyHeight)
                .frame(height: max(bodyHeight, 400))
            } else {
