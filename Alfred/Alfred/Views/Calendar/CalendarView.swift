...
//  Alfred
//
//  The Calendar tab: Apple Calendar's four views — List, Day, Week, Month — over the phone's own
//  EventKit store, with full create/edit/delete. The layout mirrors the stock app closely enough
//  that someone who's used iCal for ten years should find their hands in the right places:
//  a segmented mode switch, chevrons and Today for paging, tapping an event opens its detail.
//
...
struct CalendarView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $detailEntry) { entry in
            ...
            // Apple Calendar opens an event as a full-height modal you can swipe away — a
            // mid-screen card would make the detail feel like a stub of the real thing.
...
/// Asks before iOS does. The system prompt can only ever be shown once, so spending a screen to
/// explain what Alfred wants it for is the difference between a considered Allow and a reflexive
/// Don't Allow that can afterwards only be undone in Settings.
private struct PermissionPrompt: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Text("Alfred reads and manages your calendar from this phone — every account you've added to iOS. Nothing leaves the device.")
                .font(.system(size: 16))
                .foregroundStyle(palette.textSecondary)
...
/// Access was refused, or granted only for writing. The fix lives in Settings, so this is a
/// signpost rather than another attempt — iOS will not show the prompt a second time.
